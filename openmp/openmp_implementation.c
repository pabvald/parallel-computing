 ...
 */
	...
	for( iter=0; iter<max_iter && ! flag_stability; iter++ ) {
		...
		for( i=0; i<num_focal; i++ ) {
			...
			if ( focal[i].active == 2 ) {
				//#pragma omp atomic		//Modificación -> https://stackoverflow.com/questions/54186268/why-should-i-use-a-reduction-rather-than-an-atomic-variable
				num_deactivated++; //En el libro explica que se pueden utilizar las siguientes sentencias con atomic: x++, x--, --x, ++x, x = expresión escalar x
			}
		}

		//if(! first_activation) continue; 		//Modificación no presente en el código del leaderboard
		...
		for( step=0; first_activation && step<10; step++ )	{		//Modificación	
			...
			#pragma omp parallel
			{
				...
				for( i=1; i<mRows; i++ ) {
					for( j=1; j<mColumns; j++ ) {				
						float difference = accessMat( surface, i, j ) - accessMat( surfaceCopy, i, j ); // Modificación
		...
		for( t=0; t<num_teams; t++ ) {
			...
			for( j=0; j<num_focal; j++ ) { 
				...
				float local_square_distance = dx*dx + dy*dy;	// Modificación
			...
			if ( teams[t].type == 1 ) { 
				...
				else if ( focal[target].x > teams[t].x ) teams[t].x++;	//Modificación
				if ( focal[target].y < teams[t].y ) teams[t].y--;		
				else if ( focal[target].y > teams[t].y ) teams[t].y++;	//Modificación
			}
		...
		for( t=0; t<num_teams; t++ ) {
			...
			int squareRadius = radius*radius;			//Modificación
			...
			for( i=teams[t].x-radius; i<=xR; i++ ) {
				for( j=teams[t].y-radius; j<=yR; j++ ) {
					...
					float squareDistance = dx*dx + dy*dy;			//Modificación
